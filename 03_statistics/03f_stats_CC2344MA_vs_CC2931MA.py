"""
Motility Assay Statistical Testing – CC-2344 MA vs CC-2931 MA
-------------------------------------------------------------
Author: Ayesha Syeda
...
Purpose
-------
Compare motility (Maximum Distance) between CC-2344 and CC-2931 
Mutation Accumulation (MA) strains.

These tests assess whether motility differs between the two MA backgrounds.
Because sampling was not randomized, the results are descriptive rather than inferential.
...
Output
------
- anova_results_CC2344MA_vs_CC2931MA.csv
- permutation_results_CC2344MA_vs_CC2931MA.csv
- pairwise_results_CC2344MA_vs_CC2931MA.csv

Usage
-----
    python 03f_stats_CC2344MA_vs_CC2931MA.py
"""
...
subset = subset[subset["type"].isin(["CC-2344 MA", "CC-2931 MA"])].copy()
...
Purpose:
    ...
    CC-2344 MA and CC-2931 MA strains, adjusting for 'variable' (experimental week)
...
Interpretation:
    ...
    in average motility between CC-2344 MA and CC-2931 MA lines.
"""

model = smf.ols("value ~ C(type) + C(variable)", data=subset).fit()
anova_table = sm.stats.anova_lm(model, typ=2)
anova_table.to_csv("anova_results_CC2344MA_vs_CC2931MA.csv", index=True)
...
-----------------------------
Purpose:
    Assess whether CC-2344 MA and CC-2931 MA differ in mean motility 
...
Interpretation:
    A large p-value indicates no significant difference between the two groups.
"""
...
---------------------------------------------
Purpose:
    Compare the mean motility difference between CC-2344 MA and CC-2931 MA
    under random shuffling of observations.

Logic:
    1. Compute observed mean difference.
    2. Shuffle pooled values 10,000 times.
    3. Recompute mean difference each time.
    4. p-value = proportion of permuted differences ≥ observed.
    5. Adjust p-values using FDR correction (Benjamini–Hochberg).
"""
...
pairwise_results.to_csv("pairwise_results_CC2344MA_vs_CC2931MA.csv", index=False)
