"""
Motility Assay Statistical Testing – NI vs CC-2931 MA
-----------------------------------------------------
Author: Ayesha Syeda
...
and CC-2931 Mutation Accumulation (MA) strains.

These tests evaluate whether NI and CC-2931 MA differ in their 
average motility or distributional characteristics. Because samples 
were not randomized, the results are descriptive rather than inferential.
...
Output
------
- anova_results_NI_vs_CC2931MA.csv
- permutation_results_NI_vs_CC2931MA.csv
- pairwise_results_NI_vs_CC2931MA.csv

Usage
-----
    python 03e_stats_NI_vs_CC2931MA.py
"""
...
subset = subset[subset["type"].isin(["NI", "CC-2931 MA"])].copy()
...
-------------------
Purpose:
    Evaluate whether mean motility (Maximum Distance) differs between
    NI and CC-2931 MA strains, adjusting for 'variable' (experimental week)
...
Interpretation:
    A non-significant type term indicates no clear evidence of 
    a difference in mean motility between NI and CC-2931 MA.
"""

model = smf.ols("value ~ C(type) + C(variable)", data=subset).fit()
anova_table = sm.stats.anova_lm(model, typ=2)
anova_table.to_csv("anova_results_NI_vs_CC2931MA.csv", index=True)
...
-----------------------------
Purpose:
    Assess whether NI and CC-2931 MA differ in mean motility more
    than expected by chance.

Logic:
    1. Compute observed between-group variance (weighted by group size).
    2. Randomly shuffle type labels 10,000 times.
    3. Recompute between-group variance for each permutation.
    4. p-value = proportion of permutations producing ≥ observed variance.
...
---------------------------------------------
Purpose:
    Test whether the observed difference in mean motility between NI and
    CC-2931 MA is greater than expected by chance.

Logic:
    1. Compute the observed mean difference between groups.
    2. Shuffle pooled values 10,000 times, recomputing the difference each time.
    3. p-value = fraction of permuted differences ≥ observed.
    4. Adjust p-values using FDR correction (Benjamini–Hochberg).
"""
...
pairwise_results.to_csv("pairwise_results_NI_vs_CC2931MA.csv", index=False)
