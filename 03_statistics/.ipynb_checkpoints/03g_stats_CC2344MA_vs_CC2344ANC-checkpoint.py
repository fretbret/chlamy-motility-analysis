"""
Motility Assay Statistical Testing – CC-2344 MA vs CC-2344 ANC
--------------------------------------------------------------
Author: Ayesha Syeda
...
Purpose
-------
Compare motility (Maximum Distance) between the CC-2344 Mutation 
Accumulation (MA) strain and its ancestral (ANC) counterpart.

These tests assess whether accumulated mutations have led to 
a measurable difference in motility relative to the unmutated ancestor.
...
Output
------
- anova_results_CC2344MA_vs_CC2344ANC.csv
- permutation_results_CC2344MA_vs_CC2344ANC.csv
- pairwise_results_CC2344MA_vs_CC2344ANC.csv

Usage
-----
    python 03g_stats_CC2344MA_vs_CC2344ANC.py
"""
...
subset = subset[subset["type"].isin(["CC-2344 MA", "CC-2344 ANC"])].copy()
...
Purpose:
    ...
    CC-2344 MA and CC-2344 ANC strains, adjusting for 'variable' (experimental week)
...
Interpretation:
    A significant type effect indicates evidence of a difference in average 
    motility between the MA and ancestral strains.
"""

model = smf.ols("value ~ C(type) + C(variable)", data=subset).fit()
anova_table = sm.stats.anova_lm(model, typ=2)
anova_table.to_csv("anova_results_CC2344MA_vs_CC2344ANC.csv", index=True)
...
-----------------------------
Purpose:
    Assess whether CC-2344 MA and CC-2344 ANC differ in mean motility 
    more than expected by chance.

Logic:
    1. Compute observed between-group variance (weighted by group size).
    2. Shuffle type labels 10,000 times.
    3. Recompute variance each time.
    4. p-value = proportion of permuted variances ≥ observed variance.
...
---------------------------------------------
Purpose:
    Test whether the mean motility difference between CC-2344 MA and CC-2344 ANC 
    is greater than expected under random reassignment of values.

Logic:
    1. Compute the observed mean difference.
    2. Shuffle pooled values 10,000 times, recomputing mean differences.
...
pairwise_results.to_csv("pairwise_results_CC2344MA_vs_CC2344ANC.csv", index=False)
